        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-md-2 ml-auto">
                <div class="form-group mb-0">
                    <input type="text" class="aiz-date-range form-control"  name="date" placeholder="{{ translate('Filter by date') }}" data-format="Y-MM-DD" data-separator=" to " data-advanced-range="true" autocomplete="on">
                </div>
            </div>
        ...
        <div class="card-body">
            ...
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    <th data-breakpoints="lg">{{ translate('Allow withdrawal') }}</th>
                ...
                @foreach($shops as $key => $shop)
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_withdraw_permission(this)" value="{{ $shop->id }}" type="checkbox" <?php if($shop->can_withdraw == 1) echo "checked";?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    ...
                                    @endif
                                                                        @if($shop->user->bank_ban != 1)
                                        <a href="#" onclick="confirm_bank_ban('{{route('sellers.bank_ban', $shop->id)}}');" class="dropdown-item">
                                        {{translate('อายัดบัญชี')}}
                                        <i class="fa fa-ban text-danger" aria-hidden="true"></i>
                                        </a>
                                    @else
                                        <a href="#" onclick="confirm_bank_unban('{{route('sellers.bank_ban', $shop->id)}}');" class="dropdown-item">
                                        {{translate('ถอนอายัดบัญชี')}}
                                        <i class="fa fa-check text-success" aria-hidden="true"></i>
                                        </a>
                                    @endif
                                    ...
                                    <span onclick="show_balance({{$shop->id}})" class="dropdown-item" style="cursor:pointer;">
...
@if(is_iterable($shops) && count($shops) > 0)
    <tr>
        <td></td>
        <td></td>
        <td></td>
...
@section('modal')
	...
	<div class="modal fade" id="confirm-unban">
			...
			<div class="modal fade" id="confirm-bank-ban">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title h6">{{translate('Confirmation')}}</h5>
				...
				<div class="modal-body">
                    <p>{{translate('คุณต้องการอายัดบัญชีนี้หรือไม่?')}}</p>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-light" data-dismiss="modal">{{translate('Cancel')}}</button>
					<a class="btn btn-primary" id="confirmation_bank">{{translate('Proceed!')}}</a>
				</div>
			</div>
		</div>
	</div>
	...
	<!-- Unban Seller Modal -->
	<div class="modal fade" id="confirm-bank-unban">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title h6">{{translate('Confirmation')}}</h5>
					...
					<div class="modal-body">
							<p>{{translate('คุณต้องการถอนอายัดบัญชีนี้หรือไม่?')}}</p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-light" data-dismiss="modal">{{translate('Cancel')}}</button>
						<a class="btn btn-primary" id="confirmation_bank_unban">{{translate('Proceed!')}}</a>
					</div>
				</div>
			</div>
		</div>
    ...
    function show_balance(shop_id)
     {
        ...
        content: `<div class="row" style="width: 420px;  margin-left:7px; margin-top:10px;">
                    ...
                    <div class="col-sm-12" style="margin-top:8px;">
                        <div class="input-group">
                            <input id="inc_num" type="text" value="" class="form-control" placeholder="操作金额">
                        </div>
                    </div>
                </div>`
        ,
        btn:['{{translate('save')}}','{{translate('cancel')}}'],
        btn1: function (index,layero) {
            var inc_num = $("#inc_num").val();
            var method = $("#method").val();
              $.post('{{ route('sellers.setbalance') }}',{_token:'{{ @csrf_token() }}', shop_id:shop_id, num:inc_num, method:method}, function(data){
                layer.msg(data.msg,function(){
                    location.reload();
                });
     ...
     function show_view(shop_id,view_inc_num,view_base_num) {
        ...
        function update_comment_permission(el){
         ...
         function update_withdraw_permission(el){
            if(el.checked){
                var status = 1;
            }
            else{
            ...
            $.post('{{ route('sellers.withdraw_permission') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
        ...
        function confirm_bank_ban(url)
        {
            $('#confirm-bank-ban').modal('show', {backdrop: 'static'});
            document.getElementById('confirmation_bank').setAttribute('href' , url);
        }

        function confirm_bank_unban(url)
        {
            $('#confirm-bank-unban').modal('show', {backdrop: 'static'});
            document.getElementById('confirmation_bank_unban').setAttribute('href' , url);
        }
